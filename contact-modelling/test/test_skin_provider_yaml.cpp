...
struct MeshWrap {
  std::unique_ptr<cm::MeshInterface> p;

  cm::MeshInterface::nodes_iterator
  begin()
  {
    if (!p)
...
BOOST_AUTO_TEST_CASE(read_yaml_file)
{
  ...
  std::unique_ptr<cm::MeshInterface> m ((cm::MeshInterface*) provider.createMesh());
  provider.update(update_values);
  std::vector<double> mesh_nodes_coordinates;
  std::for_each(m->nodes_begin(), m->nodes_end(), [&] (cm::MeshInterface::node_type n) {
