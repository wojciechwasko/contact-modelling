...
MeshRegularRectangular::MeshRegularRectangular(const MeshInterface& base_mesh, const double dx, const double dy, const size_t dim)
  : MeshRegularRectangular(
      dim,
      base_mesh.minX(), 
      base_mesh.minY(),
...
MeshRegularRectangular::MeshRegularRectangular(
  const size_t dim,
  const double x0,
  const double y0,
  const double x1,
...
double 
MeshRegularRectangular::impl_node_area(size_t i) const
{
  return dx_*dy_;
}

size_t 
MeshRegularRectangular::calculate_no_nodes(double x0, double y0, double x1, double y1, double dx, double dy)
{
  if (!(x1 > x0)) {
...
size_t 
MeshRegularRectangular::calculate_no_nodes_1D(double t0, double t1, double d)
{
  if (!(t1 > t0)) {
    throw std::runtime_error(
...
double 
MeshRegularRectangular::impl_dx() const
{
  return dx_;
}

double 
MeshRegularRectangular::impl_dy() const
{
  return dy_;
}
